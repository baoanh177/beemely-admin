...
export const getColorById = createAsyncThunk("color/get-color-by-id", async (id: string, { rejectWithValue }) => {
  try {
    const { response, data } = await client.get<IColor>(prefix + `/${id}`);
    return response.status >= 400 ? rejectWithValue(messageCreator(data, dataKeys)) : data;
  } catch (error: any) {
    return rejectWithValue(messageCreator(error.response.data, dataKeys));
  }
...
export const updateColor = createAsyncThunk("color/update-color", async (payload: IThunkPayload, { rejectWithValue }) => {
  try {
    const { response, data } = await client.patch<IColor>(`${prefix}/${payload.param}`, payload);
    return response.status >= 400 ? rejectWithValue(messageCreator(data, dataKeys)) : data;
  } catch (error: any) {
    return rejectWithValue(messageCreator(error.response.data, dataKeys));
  }
...
export const deleteColor = createAsyncThunk("color/delete-color", async (id: string, { rejectWithValue }) => {
  try {
