...
export const getOrderStatusById = createAsyncThunk("orderStatus/get-order-status-by-id", async (id: string, { rejectWithValue }) => {
  try {
    const { response, data } = await client.get<IOrderStatus>(prefix + `/${id}`);
    return response.status >= 400 ? rejectWithValue(messageCreator(data, dataKeys)) : data;
  } catch (error: any) {
    return rejectWithValue(messageCreator(error.response.data, dataKeys));
  }
...
export const updateOrderStatus = createAsyncThunk("orderStatus/update-order-status", async (payload: IThunkPayload, { rejectWithValue }) => {
  try {
    const { response, data } = await client.patch<IOrderStatus>(`${prefix}/${payload.param}`, payload);
    return response.status >= 400 ? rejectWithValue(messageCreator(data, dataKeys)) : data;
  } catch (error: any) {
    return rejectWithValue(messageCreator(error.response.data, dataKeys));
  }
...
export const deleteOrderStatus = createAsyncThunk("orderStatus/delete-order-status", async (id: string, { rejectWithValue }) => {
  try {
