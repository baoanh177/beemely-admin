...
export const getCategoryById = createAsyncThunk("gender/get-gender-by-id", async (payload: IThunkPayload, { rejectWithValue }) => {
  try {
    const { response, data } = await client.get<ICategory>(prefix, payload);
    return response.status >= 400 ? rejectWithValue(messageCreator(data, dataKeys)) : data;
  } catch (error: any) {
...
export const createCategory = createAsyncThunk("gender/create-gender", async (payload: IThunkPayload, { rejectWithValue }) => {
  try {
...
export const updateCategory = createAsyncThunk("gender/update-gender", async (payload: IThunkPayload, { rejectWithValue }) => {
  try {
...
export const deleteCategory = createAsyncThunk("gender/delete-gender", async (payload: IThunkPayload, { rejectWithValue }) => {
  try {
