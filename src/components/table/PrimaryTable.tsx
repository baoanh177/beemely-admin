...
import { DefaultSearch, IDefaultSearchProp } from "../search/DefaultSearch";
...
export type IAdvancedSearch = Array<
  TextNumberField | DateField | StatusField | SwitchField | CheckField
>;
        ...
        <div>
          <div className="flex gap-4">
            ...
            {!!advancedSearch.length && (
              <div
                className={clsx("flex h-[40px] w-[48px]  text-lg shrink-0 cursor-pointer items-center justify-center rounded-lg",
                  showAdvancedSearch ? "bg-primary-50 text-primary-500 border border-primary-500" : "border-gray-100 border bg-white text-gray-400"
                )}
          ...
          {showAdvancedSearch && !!advancedSearch.length && (
            <AdvancedSearch advanced={advancedSearch} />
          )}
        </div>
      )}
      {!isTableLoading ? (
